...
walletBtn.addEventListener('click', () => {
  const isOpen = walletWrap.classList.toggle('wallet-open');
  walletBtn.setAttribute('aria-expanded', isOpen ? 'true' : 'false');
});
document.addEventListener('click', e => {
  if (!e.target.closest('.wallet-wrap') && walletWrap.classList.contains('wallet-open')) {
    walletWrap.classList.remove('wallet-open');
    walletBtn.setAttribute('aria-expanded', 'false');
  }
});

// ─── Custom gold page scrollbar ───────────────────
(function () {
  const bar = document.querySelector('.f0x-scroll:not(.f0x-scroll--mini)');
  const thumb = bar?.querySelector('.f0x-scroll-thumb');
  const track = bar?.querySelector('.f0x-scroll-track');
  if (!bar || !thumb || !track) return;
