...
// SVG is static developer markup — no user data, safe to set once
function externalLinkIcon() {
  ...
  // static SVG path — not derived from any user or API data
...
function createCard(inv) {
  ...
  const amountStat = statPair('Amount', fmtAmount(inv.amount));
  amountStat.classList.add('offer-amount');
  stats.appendChild(amountStat);
  stats.appendChild(statPair('APR',      `${inv.apr.toFixed(1)}%`));
  stats.appendChild(statPair('Due Date', `${inv.dueDays} days`));
  card.appendChild(stats);

  // Fill bar
  const fill = el('div', 'offer-fill');
  const fillHeader = el('div', 'offer-fill-header');
  fillHeader.appendChild(txt('span', 'offer-stat-label', 'Filled'));
  ...
  contribs.appendChild(statPair('Contributors',    String(inv.contributors)));
  contribs.appendChild(statPair('Min Contribution', fmtAmount(inv.minContribution)));
  card.appendChild(contribs);

  // Actions
  const actions = el('div', 'offer-actions');
  const btnContrib = el('button', 'BtnDark');
  btnContrib.appendChild(txt('span', null, 'Contribute'));
  const btnDetails = el('button', 'LightBtn');
  btnDetails.appendChild(txt('span', null, 'View Details'));
...
async function loadStats() {
  ...
  try {
    ...
    tvlValueEl.textContent = fmtTVL(data.tvl, data.currency);
    if (tvlDateEl) tvlDateEl.textContent = `TVL on ${data.tvlDate}`;
  } catch {
...
async function loadInvoices() {
  ...
  try {
    ...
    if (!top3.length) {
      ...
      empty.textContent = 'No active offers at this time.';
    ...
    err.textContent = 'Failed to load offers. Please try again later.';
...
(function () {
  ...
  function sync() {
    ...
    // Fill represents how far scrolled; thumb width = 1/cards fraction
