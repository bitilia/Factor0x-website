import { t, dealField, formatMoney } from './i18n.js?v=demo3';
import { getDeal } from './deals-store.js?v=demo3';
...
function fallbackMaturityDate(dueDays) {
  const d = new Date(Date.now() + (dueDays || 0) * 86400000);
  return d.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
}

/**
 * Normalizes any registered invoice (offer-card shape or invoice-table-row
 * shape) into the flat field set the modal markup expects. Curated deals
 * (INV-041/088/113) get their locale copy from js/locales/*; every other
 * placeholder invoice gets sensible generated fallbacks so "View Details"
 * works for all 20+ rows, not just the three cards on the hero grid.
 */
function normalizeDeal(inv) {
  const id = inv.id;
  const riskLabel = inv.risk === 'medium' ? t('modal.riskMedium') : t('modal.riskLow');
  return {
    id,
    company: inv.company,
    ref: dealField(id, 'ref') || `#${id} · ${inv.route || ''}`,
    description: dealField(id, 'description') ||
      `Verified B2B invoice from ${inv.company}, assessed at ${riskLabel.toLowerCase()} risk with a ${inv.dueDays}-day repayment term.`,
    sector: dealField(id, 'sector') || inv.route || '—',
    jurisdiction: dealField(id, 'jurisdiction') || 'UAE',
    maturityDate: dealField(id, 'maturityDate') || fallbackMaturityDate(inv.dueDays),
    apr: inv.apr,
    filled: inv.filledPct,
    investors: inv.contributors,
    totalAmount: inv.amount,
    dueDays: inv.dueDays,
    minContribution: inv.minContribution,
    obligor: inv.obligor || inv.company,
    riskLabel,
    image: inv.image ? `${inv.image}?v=demo3` : '',
  };
}
...
function buildModal() {
  ...
  el.innerHTML = `
    <div class="modal-wrap">
      ...
      <div class="modal-left">
        ...
        <div class="modal-left-panel f0x-scroll-host">
        <div class="modal-left-body" data-f0x-scroll>
      ...
      <div class="modal-right">
        <div class="modal-right-panel f0x-scroll-host">
        <div class="modal-right-body" data-f0x-scroll>
  ...
  initMiniScrollbar(el.querySelector('.modal-box'));
  initMiniScrollbar(el.querySelector('.modal-left-body'));
  initMiniScrollbar(el.querySelector('.modal-right-body'));
...
function showModal(inv) {
  ...
  const riskEl = modalEl.querySelector('#m-risk');
  if (riskEl) {
    const riskCls = d.riskLabel === t('modal.riskMedium') ? 'badge-blue' : 'badge-emerald';
    riskEl.innerHTML = `<span class="badge ${riskCls} badge--sm">${d.riskLabel}</span>`;
  }
...
document.addEventListener('click', e => {
  const btn = e.target.closest('.offer-actions .LightBtn, .invoice-table-actions .LightBtn');
  if (!btn) return;
  const row = btn.closest('[data-id]');
  if (!row) return;
  const inv = getDeal(row.dataset.id);
  if (inv) showModal(inv);
});
